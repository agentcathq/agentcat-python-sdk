...
- **The SDK never touches your process lifecycle.** 2.0.0 beta builds replaced `SIGINT`/`SIGTERM` handlers at import and force-exited via `os._exit(0)` after a drain delay — clobbering `KeyboardInterrupt`, your own handlers, `finally` blocks and `atexit` hooks. Stable 2.0 installs no signal handlers, registers no exit-time event drain, and runs every worker as a daemon thread: your shutdown is entirely yours, and exit is never delayed by AgentCat. The trade is deliberate: telemetry still queued when the process exits is dropped. The one exit hook that remains is the internal-diagnostics beacon — skipped when empty, capped at ~2 seconds. Idle workers park in untimed waits, so interpreter exit never wakes them — on Linux with CPython ≤3.13, a daemon thread waking from a timed wait during finalization aborts the whole process (CPython gh-87135), which is exactly the corruption this design avoids. One narrow residue of that CPython behavior remains: a publish retrying against an unreachable API holds a short timed backoff wait, and a process that exits during that window can still hit the abort. The window only exists while the AgentCat API is down, and Python 3.14 removes the underlying behavior.
